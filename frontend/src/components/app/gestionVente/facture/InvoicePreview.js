...
const InvoicePreview = ({
  ...
  const total = (items || []).reduce((acc, item) => {
    const unitPrice =
      item.prixVente ||
      Math.floor((item.totalPrice || 0) / (item.quantity || 1)) ||
      0;
        ...
        <div className="invoice-preview">
          ...
          <div className="row g-3 mb-3">
            <div className="col-12 col-md-4 ms-auto">
              <div
                className={`card ${
                  isDark ? 'bg-secondary border-secondary' : 'border'
                }`}
              >
                <div
                  className={`card-header py-1 text-center fw-semibold ${
                ...
                <div className="card-body py-2">
                  ...
                  <div>
                    Client: {customerInfo.phoneNumber || 'Non spécifié'}
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="d-flex justify-content-end my-3">
            <div
              className={`card ${
                isDark ? 'bg-secondary border-secondary' : ''
              }`}
