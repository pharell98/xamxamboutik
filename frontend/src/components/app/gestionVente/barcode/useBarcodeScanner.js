...
const NAVIGATION_KEYS = [
  ...
  'Home', 'End', 'PageUp', 'PageDown', 'Meta', 'Alt', 'Ctrl', 'Shift',
  'CapsLock', 'NumLock', 'ScrollLock',
  ...Array.from({ length: 12 }, (_, i) => `F${i + 1}`)
];

const BLOCKED_SELECTORS = [
  '.cart-section', '.cart-item', '.quantity-controller', '.price-wrapper',
  '.modal', '[role="dialog"]', '.popover', '.dropdown-menu',
  '.navbar', '.nav', '.sidebar', '.navbar-vertical', '.navbar-top'
];

const INPUT_TYPES = ['text', 'number', 'tel', 'email', 'password', 'search', 'url'];
...
const useBarcodeScanner = (onScan, isActive = true) => {
  ...
  const isElementBlocked = useCallback(element => {
    ...
    // Éléments de saisie
    const isInputElement = ['INPUT', 'TEXTAREA', 'SELECT'].includes(element.tagName) ||
                          element.contentEditable === 'true' ||
                          INPUT_TYPES.includes(element.type);

    // Éléments interactifs
    const isInteractiveElement = ['BUTTON', 'A'].includes(element.tagName) ||
    ...
    return isInputElement || isInteractiveElement || isInBlockedContainer || isInForm;
  }, []);
  ...
  const handleBarcodeInput = useCallback((key) => {
    if (key === 'Enter') {
  ...
  const handleKeyDown = useCallback((event) => {
    ...
    if (isElementBlocked(activeElement) && activeElement !== containerRef.current) {
      return;
    }
  ...
  useEffect(() => {
    ...
    const handleGlobalKeyDown = (event) => {
      ...
      if (NAVIGATION_KEYS.includes(key) || isElementBlocked(activeElement)) return;
