...
const FormField = React.memo(
  ({ name, methods, placeholder = '', type = 'text', onChange }) => {
    const {
      register,
      formState: { errors }
    } = methods;
...
const ProductForm = React.memo(
  ({
    ...
    // Mémorisation des callbacks pour éviter les re-créations
    ...
    }, [shouldResetBaseFields, reset, onResetDone]);
    ...
    useEffect(() => {
      const fetchSuggestions = async query => {
        ...
        try {
          const data = await apiServiceV1.getApprovisionnementSuggestions(
            query
          );
